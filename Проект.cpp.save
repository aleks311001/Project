       ...
       00 ( ) 00999999999999999)
        00 " 0099999999999999999
         00000999999999999999999
           ...
           99     99    99     99
          999    999   999    999
тезисы пол стр а4 10-12 шрифт примерно 400 слов, план презентации
...
void text (char * text, double x, double y, double shrift = 15, double a = 0);
...
void Risovanie (double do_shap_iz, double Dlina_iz, int Kol_vo_iz, double Diam_iz, double Stroit_vis)
    ...
    DrawOpora (Opor_kord, Zoom, 400, 1740);
    Iz_vmeste (Opor_kord.x_travers11, Opor_kord.y_travers11, Opor_kord.x_travers12, Opor_kord.y_travers12, Zoom, do_shap_iz, Dlina_iz,
               Stroit_vis, Kol_vo_iz, Diam_iz, 320 + n, "1");
    Iz_vmeste (Opor_kord.x_travers21, Opor_kord.y_travers21, Opor_kord.x_travers22, Opor_kord.y_travers22, Zoom, do_shap_iz, Dlina_iz,
               Stroit_vis, Kol_vo_iz, Diam_iz, 250 + n, "2");
    Iz_vmeste (Opor_kord.x_travers31, Opor_kord.y_travers31, Opor_kord.x_travers32, Opor_kord.y_travers32, Zoom, do_shap_iz, Dlina_iz,
               Stroit_vis, Kol_vo_iz, Diam_iz, 180 + n, "3");
    Iz_vmeste (Opor_kord.x_travers34, Opor_kord.y_travers34, Opor_kord.x_travers33, Opor_kord.y_travers33, Zoom, do_shap_iz, Dlina_iz,
               Stroit_vis, Kol_vo_iz, Diam_iz, 110 + n, "4");
    Iz_vmeste (Opor_kord.x_travers24, Opor_kord.y_travers24, Opor_kord.x_travers23, Opor_kord.y_travers23, Zoom, do_shap_iz, Dlina_iz,
               Stroit_vis, Kol_vo_iz, Diam_iz, 40  + n, "5");
    Iz_vmeste (Opor_kord.x_travers14, Opor_kord.y_travers14, Opor_kord.x_travers13, Opor_kord.y_travers13, Zoom, do_shap_iz, Dlina_iz,
               Stroit_vis, Kol_vo_iz, Diam_iz,-30 + n, "6");
...
void Iz_Naklon (double x, double y, double x2, double y2, double *Ugol_max, double Zoom, double do_shap_iz, double Stroit_vis,
    ...
    txSelectFont ("Arial", 55, 22, 0);
    if (x > 0) txTextOut (x * Zoom + perX + 35, -y * Zoom + perY, number);
    if (x < 0) txTextOut (x * Zoom + perX - 35, -y * Zoom + perY, number);
...
void Otobragenie_Ballast (double Ugol_max, int x, int y)
    ...
    txSelectFont ("Arial", 55, 20.4, 0);
    txTextOut (x - 355, y - 27, text);
    }
...
void Iz_vmeste (double x, double y, double x2, double y2, double Zoom, double do_shap_iz, double Dlina_iz,
    ...
    if (x != x2)
        ...
        double Ugol_max = 0;
        txSetColor (TX_BLACK, 3);
        Iz (x, y, Zoom, do_shap_iz, Dlina_iz, Kol_vo_iz, Diam_iz, Stroit_vis, 400, 1740);
        Iz_Naklon (x, y, x2, y2, &Ugol_max, Zoom, do_shap_iz, Stroit_vis, Dlina_iz, Kol_vo_iz, Diam_iz, 400, 1740, number);
        Otobragenie_Ballast (Ugol_max, XWindow - 356, YWindow - y_b);
        }
    }
...
void Text_Stroka (double x, double y, bool *up_or_down, double* vvod, double* pos)
    ...
    for (;; !GetAsyncKeyState (VK_RETURN) || !GetAsyncKeyState (VK_UP) || !GetAsyncKeyState (VK_DOWN))
        ...
        if (GetAsyncKeyState (VK_RETURN) || GetAsyncKeyState (VK_DOWN))
            {
            *up_or_down = false;
            break;
            }
